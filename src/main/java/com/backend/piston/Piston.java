...
import com.backend.exception.PistonException;
import com.backend.utils.PistonUtils;
...
public class Piston {
    ...
    private void initRuntimes() {
        initialisationThread = new Thread(() -> {
            try {
                ...
                String json = PistonUtils.get(url);
                Gson gson = new Gson();
    ...
    public ExecutionResult execute(ExecutionRequest request) {
        ...
        try (BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(con.getInputStream(), StandardCharsets.UTF_8))) {
            ...
            if (errorStream != null) {
                String error = PistonUtils.readStream(errorStream);
